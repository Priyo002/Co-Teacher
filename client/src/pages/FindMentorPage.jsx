              ...
              paginatedMentors.map(mentor => (
                ...
                <div className="w-full md:w-48 h-48 rounded-xl overflow-hidden shrink-0 bg-slate-100 border border-slate-200">
                  {mentor.profilePicture ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center font-bold text-5xl text-brand-500 bg-brand-50">
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-sm text-slate-500 mt-1">
                        <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {mentor.mentorProfile?.location || 'Remote'}</span>
                        <span>•</span>
                        <span className="flex items-center gap-1"><Users className="w-4 h-4" /> {mentor.mentorProfile?.languages?.join(', ') || 'English'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {mentor.mentorProfile?.skills?.slice(0, 6).map((skill, i) => (
                      <span key={i} className="text-xs font-semibold bg-slate-100 text-slate-700 px-3 py-1.5 rounded-full border border-slate-200">
                        {skill}
                      </span>
                    ))}
                    {mentor.mentorProfile?.skills?.length > 6 && (
                      <span className="text-xs font-semibold text-brand-600 px-2 py-1.5 cursor-pointer">
                  ...
                  <div className="mt-6 flex flex-wrap gap-4">
                    <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex-1 min-w-[200px]">
                      <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600"><Briefcase className="w-4 h-4" /></div>
                      <div>
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex-1 min-w-[200px]">
                      <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600"><Star className="w-4 h-4" /></div>
                      <div>
                        <p className="text-sm font-bold text-slate-900">{mentor.mentorProfile?.experienceYears || 0}+ Years of Exp.</p>
                        <p className="text-xs text-slate-500">Industry Veteran</p>
                      </div>
                    </div>
                  </div>
                </div>
